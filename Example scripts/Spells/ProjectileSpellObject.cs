...
public class ProjectileSpellObject : ProjectileObject
{
    ...
    readonly float effectStrength = 1f;
    readonly float maxTrackingSpeed = 15f;
    readonly float maxLifespan = 15f;
    ...
    public void initializeSpell(Rune rune1, Rune rune2)
    {
        ...
        Damage damage = new Damage("player_projectile", modifiedDamageInt, rune1, rune2, false);
        ...
        Damage damageCrit = new Damage("player_projectileCrit", modifiedDamageIntCrit, rune1, rune2, false);
    ...
    public override void TrackLocation(EnemyStats target)
    {
        ...
        if (!isTracking)
        {
            ...
            float distanceScaler = 2f - Mathf.Clamp(Vector3.Distance(transform.position, ContinousMovement.rig.centerEyeAnchor.position), 0f, 2f);
...
{
    public abstract void TrackLocation(EnemyStats target);
}
